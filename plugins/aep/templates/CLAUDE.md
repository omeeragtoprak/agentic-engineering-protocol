...
- **Skills (invoke with `/` or auto-match):** `aep:protocol` orchestrates the full loop; `aep:explore`, `aep:plan`, `aep:implement`, `aep:verify`, `aep:deliver` are the phase playbooks; `aep:standards` is the security/performance/testing reference; `aep:research` is the triangulated web-research protocol; `aep:orchestrate` is the subagent-delegation playbook. `/aep:status` reports where the project stands from evidence. Prefer invoking the phase skill over improvising the phase.
