...
5. **Deliver.** Summarize what changed and why, verification evidence, remaining trade-offs/risks, follow-ups needing a human decision. Close every requirement you touched in `.claude/requirements.md` — `done` with a named proof, or `deferred`/`dropped` with a date and a reason; a deferral that lives only in a summary is lost. Atomic, intent-revealing commits (Conventional Commits unless §P.3 says otherwise); never commit unrelated files or a red build; never force-push shared branches. Update §P if durable knowledge was produced (rules in §5).
