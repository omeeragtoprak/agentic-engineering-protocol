...
5. **Deliver.** Summarize what changed and why, verification evidence, remaining trade-offs/risks, follow-ups needing a human decision. If `.claude/requirements.md` exists, **write this task into it before you finish** — one row for what the task committed to, `done` with a named proof, or `deferred`/`dropped` with a date and a reason. A task that committed to nothing says so in one line. (Closing rows you did not open is not enough: measured across twelve sessions, a ledger nobody adds to stays empty.) Atomic, intent-revealing commits (Conventional Commits unless §P.3 says otherwise); never commit unrelated files or a red build; never force-push shared branches. Update §P if durable knowledge was produced (rules in §5).
